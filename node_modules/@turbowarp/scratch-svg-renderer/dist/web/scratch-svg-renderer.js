 ...
 */
const inlineSvgFonts = function inlineSvgFonts(svgString, customFontFaces = {}) {
    ...
    if (customFontFaces && typeof customFontFaces === 'object') {
        ...
        for (const _ref of Object.entries(customFontFaces)) {
            var _ref2 = _slicedToArray(_ref, 2);

            const families = _ref2[0];
            const style = _ref2[1];
...
const fixForVanilla = __webpack_require__(/*! ./tw-fix-for-vanilla */ "./src/tw-fix-for-vanilla.js");
...
const getSandbox = __webpack_require__(/*! ./tw-svg-sandbox */ "./src/tw-svg-sandbox.js");
...
const extraMetadataRegex = new RegExp(`${HTML_COMMENT_START}rotationCenter:(-?[\\d\\.]+):(-?[\\d\\.]+)${HTML_COMMENT_END}$`);
        ...
        switch (lower) {
            ...
            case 's':
                ...
                for (let j = 0; j < length; j += 4) {
                    const handle1 = /[cs]/.test(previous) ? { x: current.x * 2 - control.x, y: current.y * 2 - control.y } : current;
                    control = getPoint(j);
...
const _createGradient = function _createGradient(gradientId, svgTag, bbox, matrix) {
    ...
    if (radial) {
        ...
        let gradientPerpendicular = Math.abs(origin.x - destination.x) < 1e-8 ? add(origin, { x: 1, y: (origin.x - destination.x) / (destination.y - origin.y) }) : add(origin, { x: (destination.y - origin.y) / (origin.x - destination.x), y: 1 });
        ...
        const gradientDirection = Math.abs(originToPerpendicular.x) < 1e-8 ? { x: 1, y: -originToPerpendicular.x / originToPerpendicular.y } : { x: -originToPerpendicular.y / originToPerpendicular.x, y: 1 };
