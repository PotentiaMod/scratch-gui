...
function applyBackgroundLayout({ image, containerWidth, containerHeight, mode = 'stretch', offsetX = 0, offsetY = 0 }) {
...
async function getWallpaperRotationList(settings = null) {
  ...
  const savedList = Array.isArray(resolvedSettings.WallpaperRotationList)
    ? resolvedSettings.WallpaperRotationList
    : null;
...
export default async function ({ addon, msg }) {
  ...
  while (true) {
    const elem = await addon.tab.waitForElement(
      'div[class*="menu-bar_file-group"] > div:last-child:not(.sa-background)',
      {
        markAsSeen: true,
        reduxEvents: [
...
function showBgModal(addon, msg) {
  const { backdrop, container, content, closeButton, remove } = addon.tab.createModal(msg('background-title'), {
    isOpen: true,
        ...
        (file, index) =>
          new Promise((resolve, reject) => {
            ...
            reader.onload = async (loadEvent) => {
              try {
                const wallpaperId = isRotationEnabled
                  ? `WorkSpaceBG-${Date.now()}-${index}`
                  : currentId || `WorkSpaceBG-${Date.now()}-${index}`;
...
async function addModalBackground() {
  try {
    ...
    document
      .querySelectorAll('[class*="library_library-scroll-grid"]')
      .forEach((ele) => (ele.style.background = 'transparent'));
